...
class lunar:
    ...
    def getCnDate(self, _date):
        ...
        """
        (_year, _month, _day) = self._getNumCnDate(_date)
        return "农历 %s年 %s年%s%s %s " % (self._cnYear(_year), self._upperYear(_date), self._cnMonth(_month), self._cnDay(_day),self._upperWeek(_date))

    def getCnYear(self, _date):
        """ 获取农历年份
            Args:
                _date = datetime(year, month, day)
            Return:
                "x月"
        """
        _year = self._getNumCnDate(_date)[0]
        return "%s年" % self._cnYear(_year)
    ...
    def getUpperWeek(self, _date):
        return self._upperWeek(_date)